...
namespace parser
{
    ...
    cnf::Rule* MSResParser::next_rule()
    {
        ...
        if (line[0] == 't') {
            ...
            if (type == "msres") {
                ...
                return parseResRule(rest);
            }

            if (type == "split") {
                std::string rest = iss.str().substr(iss.tellg());
                return parseSplitRule(rest);
            }
        }
    ...
    cnf::Rule* MSResParser::parseResRule(const std::string &line) {
    ...
    cnf::Rule* MSResParser::parseSplitRule(const std::string &line) {
        // Regex matches the pattern "< clause | pivot >"
