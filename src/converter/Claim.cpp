...
namespace converter {
    ...
    Claim::Claim(
        ...
        bool negated_pivot
    ) : negated_pivot(negated_pivot), variable_supplier(variable_supplier) {
        if (clauses.size() < 3) {
        ...
        this->active_clauses = { clauses[3].second };
        ...
        if (negated_pivot) {
            ...
            for (size_t i = 0; i < num_clauses_2; i++) {
                this->active_clauses.push_back(clauses[i + 4].second);
            }
        } else {
            ...
            for (size_t i = 0; i < num_clauses_1; i++) {
                this->active_clauses.push_back(clauses[clauses.size() - 1 - i].second);
            }
        }
    }
    ...
    void Claim::set_active_constraints(const std::vector<constraintid> &active_constraints) {
        ...
        if (active_constraints.size() != active_clauses.size()) {
            throw std::runtime_error("Active constraints size does not match the number of clauses.");
        }

        for (size_t i = 0; i < active_constraints.size(); i++) {
            if (clause_map.find(active_constraints[i]) == clause_map.end()) {
                clause_map[active_constraints[i]] = active_clauses[i];
            } else {
                throw std::runtime_error("Constraint ID already exists in the clause map.");
            }
        }
    }
    ...
    // TODO: This should be optimized to avoid creating a new unordered_set for each call.
    constraintid Claim::weaken_all_except(Prooflogger &pl, constraintid id, const std::vector<Lit> &variables, uint32_t begin, uint32_t end) {
        ...
        for (int i = 0; i < variables.size(); i++) {
            if ((i < begin || i > end)) {
                if (clause_map[id].has_double_literal()) {
                    std::unordered_set<Var, VarHash, VarEqual> duplicate_vars;
                    for (auto& lit : clause_map[id].get_duplicate_literals()) {
                        duplicate_vars.insert(variable(variable_supplier(lit)));
                    }

                    if (duplicate_vars.find(variable(variables[i])) != duplicate_vars.end()) {
                        continue;
                    }
                }
    ...
    bool Claim::is_tautology(constraintid id) const {
        auto it = clause_map.find(id);
        if (it != clause_map.end()) {
            return it->second.is_tautology();
        }
        return false;
    }
}